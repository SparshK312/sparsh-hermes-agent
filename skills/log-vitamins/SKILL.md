...
2. **Write via `vault_log.py vitamins` run through `terminal` — the ONE and ONLY write.** Direct `patch` / `write_file` / `execute_code` on the daily note are BLOCKED by a guard; don't attempt them. **Log immediately — no confirmation prompt** — then relay `vault_log`'s confirmation line (don't recompute it).
