...
3. **Duplicate-meal-same-time — now blocked by the writer, don't fight it.** `vault_log.py food` fingerprints each meal (type + item names + rounded kcal/protein) and refuses an identical one logged within 20 minutes, writing NOTHING and returning `"duplicate": true` with a `↩︎ Already logged…` message. Relay that message as-is; do not retry, do not re-log with a tweaked value to get around it, and do not `--force` on your own initiative. Only pass `--force` if Sparsh explicitly says it was a genuine second serving. This exists because compaction used to re-inject his verbatim message and the same meal got logged twice — he had to audit every log by hand ("U double logged the dinner, fix it" / "You didn't double log anything right?").
...
8. **Duplicate / correction ambiguity (beyond the 20-min window).** The writer's fingerprint gate (pitfall 3) only covers a *recent* identical meal. If he re-sends a meal from earlier today — a new photo of the same plate, or the same item list hours later — the gate will NOT catch it, so ask whether it's a correction or an addition. If it's a correction, `undo-last-meal` then re-log; never double-count silently.
...
9b. **Post-confirmation correction on multi-meal retrospective logs.** If the user reports 3+ meals in one message (full day), logs all of them, then says one item was wrong (e.g. "I didn't drink fairlife"), `undo-last-meal` removes from the END of the food log, not the meal they meant to correct. If breakfast needs fixing but lunch and dinner are already logged, you must undo backward (dinner first, then lunch, then breakfast), then re-log lunch and dinner again. This is correct behavior — `undo-last-meal` is deterministic. Surface the flow clearly so the user sees you're walking backward intentionally, not fumbling. (Learned 2026-07-16.)
...
15b. **Upward bias for restaurant platter estimates.** When a user reports a chain restaurant meal by name without exact portion info (e.g., "Osmows chicken and rice platter with garlic and tzatziki"), bias the estimate UPWARD, not conservative. A "chicken and rice" read literally might be 530 cal (component sum), but the full platter with sauces and actual portions is typically 700–850 kcal. Include sauce macros in the estimate. Flag `source: estimated (full platter)` so the user can correct down if they ordered a smaller or lighter version. (Learned 2026-07-15: underestimating pizza by 45% because I was too conservative on sauce/portion weight.)

15c. **Portion-size language signals upward bias — home-cooked too.** When Sparsh describes a meal with language like "full plate", "filling meal", "heaping", or "loaded", these are NOT neutral descriptions — they signal a substantial serving, above baseline assumptions. Apply upward bias (30–50% portion bump) to home-cooked meals with this language, not just restaurants. Example: "full plate of rice with rajma" is NOT "1 cup rice + ½ cup beans" (225 kcal estimate); it is "1.5 cups rice + 1 cup beans" (~640 kcal). Home-cooked rice + legume combos especially tend to be underestimated when component-wise logic is applied to casual serving descriptions. Estimate high first; if the user says the log feels too high after the fact ("that was less than I logged"), undo and re-log with the reduction. Do NOT ask for clarification before logging. See `references/portion-size-language-and-home-meals.md` for the detailed pattern and home-meal defaults. (Learned 2026-07-16: User corrected rice + rajma from 150 kcal to 644 kcal after logging the initial underestimate.)
...
17. **Conservative portion bias.** If the user says the estimate feels a little large but otherwise accurate, tighten the portion to the smaller plausible amount rather than re-anchoring at the midpoint. Keep the same food identity unless the user explicitly changes the item list.
18. **Branded cereals / packaged items miss in MCP — don't chain queries, web-search.** Branded breakfast cereals (Vector, Honey Nut Cheerios, etc.) have ~0 MCP coverage. When MCP misses on a branded/packaged item, DON'T fire multiple MCP queries hoping for a hit — jump straight to `web_search "[Brand] [Product] nutrition facts"` for the official per-serving macros and scale to the stated portion. (Learned 2026-07-02.)
19. **Vision vs. reported mismatch — ask, don't silently re-log.** If photo analysis returns item set A but the user says they ate item set B, do NOT silently undo + re-log. Ask once ("Photo showed [X], but you said [Y] — logging [Y]?") to surface a misidentification before it diverges the vault from reality. (Learned 2026-07-02.)
20. **Full restaurant platters vs. component estimates.** When the user reports a chain restaurant meal like "Osmows chicken and rice platter with garlic and tzatziki sauces" (not just "chicken"), distinguish between: (a) a single component (e.g., just the chicken piece) and (b) the full platter (chicken + rice + sauces). MCP searches and copycat-recipe videos often estimate lower (530 cal) because they assume smaller portions or fewer sauce inclusions. **For a full platter with sauces, bias upward.** Osmows full chicken + rice + sauces ≈ 700–850 kcal, 50–65g protein (not 530). Web-search the chain's official nutrition calculator or similar chains' published data first; if unavailable, default to upper-range estimates and flag `source: estimated (full platter)` so the user can correct down if they ordered a smaller version. (Learned 2026-07-14.)
